...
class TransformerConfig:
  ...
  num_layers: int = 6
  qkv_dim: int = 16#512
  mlp_dim: int = 256#2048
  max_len: int = 2048
  dropout_rate: float = 0. #0.1 JT: setting to zero for now
  attention_dropout_rate: float = 0.#0.1
...
class Decoder(nn.Module):
  ...
  def __call__(
    ...
    # JT: Don't want any right shift here
  ...
  def __call__(self, inputs, inputs_positions=None, inputs_segmentation=None, intervention=lambda x,layer:x):
    ...
    if config.decode:
      ...
      decoder_mask = nn.combine_masks(
          #JT commenting this out since 0 is a non padding token
    ...
    logits, act = Decoder(config=config, shared_embedding=None, name='decoder')(
        inputs,
